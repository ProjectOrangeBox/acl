...
- **Passwords are always hashed** (`password_hash()`, `PASSWORD_DEFAULT`) — at create and through `updatePassword()`, which enforces the same password rules as create before hashing.
- **`User::change()` regenerates the session id** before switching users (fixation defense) — and `logout()` routes through it, so logging out regenerates too. Skipped automatically when no session is active (CLI). Importing pre-hashed passwords is a SQL job, not an API one.
