...
class UserModel extends Model implements UserModelInterface
{
    ...
    public function create(array $columns): UserEntityInterface
    {
        ...
        // 'create' (not 'update') - metaColumns has no id yet, it's only assigned below
        // once the user row (and its id) exists
        try {
            $this->userMetaModel->validateFields('create', $metaColumns);
    ...
    public function read(int $userId): UserEntityInterface
    {
        ...
        $userEntity = $this->sql->setFetchMode($this->entityClass, [$this->config, $this])->select()->from($this->tablename)->where('id', '=', $userId)->execute()->row();
